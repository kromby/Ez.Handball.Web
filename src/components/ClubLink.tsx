...
export function ClubLink({ clubId, name, fallback = "—", className, title, children }: ClubLinkProps) {
  const content = children ?? (name || fallback);
  if (clubId && clubId.length > 0) {
    const cls = ["club-link", className].filter(Boolean).join(" ");
    return (
      <Link to={clubPath(clubId)} className={cls} title={title}>
        {content}
      </Link>
    );
  }
