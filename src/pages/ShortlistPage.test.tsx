...
test("renders the shortlisted players and the count header", async () => {
  ...
  vi.spyOn(api, "getPlayerPool").mockResolvedValue(emptyPool);
...
test("renders a Buy button for a shortlisted player found in the pool", async () => {
  ...
  vi.spyOn(api, "getPlayerPool").mockResolvedValue({
    ...
    entries: [{ rank: 1, playerId: "p1", name: "Vik", clubId: "1", clubName: "Aalvik", gender: "karlar", position: "LB", price: { amount: 9_000_000, currency: "ISK" }, rating: 30, pickPercentage: null }],
  });
  renderWithProviders(
